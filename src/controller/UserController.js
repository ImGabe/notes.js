"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var typeorm_1 = require("typeorm");
var class_validator_1 = require("class-validator");
var User_1 = require("../entity/User");
// interface ResponseInternalError {
//   details: string,
//   code: number,
//   type: string
// }
var UserController = /** @class */ (function () {
    function UserController() {
    }
    UserController.prototype.all = function (_request, response, next) {
        return __awaiter(this, void 0, void 0, function () {
            var userRepository;
            return __generator(this, function (_a) {
                userRepository = typeorm_1.getConnection().getRepository(User_1.User);
                userRepository
                    .find({ relations: ["notes"] })
                    .then(function (note) { return response.json(note); })["catch"](function (err) { return next(err); });
                return [2 /*return*/];
            });
        });
    };
    UserController.prototype.one = function (request, response, next) {
        return __awaiter(this, void 0, void 0, function () {
            var userRepository, uuid;
            return __generator(this, function (_a) {
                userRepository = typeorm_1.getConnection().getRepository(User_1.User);
                uuid = request.params.uuid;
                userRepository
                    .findOneOrFail({ uuid: uuid })
                    .then(function (note) { return response.json(note); })["catch"](function (err) { return next(err); });
                return [2 /*return*/];
            });
        });
    };
    UserController.prototype.save = function (request, response, next) {
        return __awaiter(this, void 0, void 0, function () {
            var userRepository, _a, username, email, password, user, validations, errors;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        userRepository = typeorm_1.getConnection().getRepository(User_1.User);
                        _a = request.body, username = _a.username, email = _a.email, password = _a.password;
                        user = userRepository.create({ username: username, email: email, password: password });
                        return [4 /*yield*/, class_validator_1.validate(user)
                            // FIXME
                        ];
                    case 1:
                        validations = _b.sent();
                        // FIXME
                        if (Array.isArray(validations) && validations.length) {
                            errors = validations.map(function (_a) {
                                var constraints = _a.constraints;
                                return Object.values(constraints);
                            });
                            response.status(400);
                            response.json({ "error": errors });
                        }
                        userRepository
                            .save(user)
                            .then(function (user) { return response.json(user); })["catch"](function (err) { return next(err); });
                        return [2 /*return*/];
                }
            });
        });
    };
    UserController.prototype.remove = function (request, response, next) {
        return __awaiter(this, void 0, void 0, function () {
            var userRepository, uuid, userToRemove;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        userRepository = typeorm_1.getConnection().getRepository(User_1.User);
                        uuid = request.params.uuid;
                        return [4 /*yield*/, userRepository.findOne({ where: { uuid: uuid } })];
                    case 1:
                        userToRemove = _a.sent();
                        // FIXME: delete notes before !!!
                        userRepository
                            .remove(userToRemove)
                            .then(function (user) { return response.json(user); })["catch"](function (err) { return next(err); });
                        return [2 /*return*/];
                }
            });
        });
    };
    return UserController;
}());
exports.UserController = UserController;
